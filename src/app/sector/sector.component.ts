...
export class SectorComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.companyService.getRecords().subscribe(
      (resp) => {
        this.data = resp;
        this.uniqueSector = this.getUniqueSector(this.data);
        console.log(this.uniqueSector);
      }
    );
 ...
 private processFilteredData(): void {
  ...
  this.dataAfterSectorFilter.forEach((item: any) => {
    if (item[0] !== "") {
 ...
 onFilterChange(event: any): void {
  ...
  if (filterValue === 'all') {
    ...
    this.companyService.getDataByFilter("sector",filterValue).subscribe(
      (resp) => {
        console.log(resp);
        this.dataAfterSectorFilter = resp;
 ...
 private getUniqueSector(data: any[]): string[] {
